...
func handleCmdA(w io.Writer, args []string) error {
	var v string
	fs := flag.NewFlagSet("cmd-a", flag.ExitOnError)
	fs.SetOutput(w)
	fs.StringVar(&v, "verb", "default", "usage")
	err := fs.Parse(args)
	if err != nil {
		return err
	}
	fmt.Fprintf(w, "Executing command A: %s\n", v)
	return nil
}
...
func handleCmdB(w io.Writer, args []string) error {
	...
	fmt.Fprintf(w, "Executing command B: %s\n", v)
	return nil
}
